...
# List 1: continuous columns with more than 30 percent missing values (columns to be removed in cleaning part)
...
cat_with_less_than_10_per <- names(df)[(percentage_missing < 10 & unique_values > 5 | unique_values < 6 & percentage_missing < 1) 
...
transform_into_missing <- c(names(df)[unique_values < 6 & percentage_missing >= 1 | (unique_values < 300 & percentage_missing >= 10)], cont_list, cont_with_more_than_30_per)
...
#we will remove rows with missing values if it is a categorical predictor and has less than 10% missing values and 6levels or more
#remove continuous predictors with more than 30% missing data
