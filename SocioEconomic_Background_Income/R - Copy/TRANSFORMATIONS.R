...
#for both models, we will standardize all continuous columns so that they are easier to interpret in terms of their strength (except for the target)
#we also remove the previously log-transformed variables
numeric_cols<-names(main_model)[sapply(main_model, is.numeric)]
numeric_cols_no_target<- setdiff(numeric_cols, c("log_of_W1GrssyrHH(Gross annual salary of household)", "log_of_W1GrssyrMP(Gross annual salary of main parent)", 
                                                 "W8DINCW(Continuous weekly income)", "log_of_W8QDEB2(Total amount owe)"))
numeric_cols_no_target
numeric_cols <- names(main_model)[sapply(main_model, is.numeric)]
numeric_cols

# Use the function from lectures
standardise.fn <- function(v) { (v - mean(v, na.rm=T)) / sd(v, na.rm=T) }
