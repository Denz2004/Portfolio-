...
# List 3: categorical columns with more than 10% missing values (we will transform the rows in the cleaning part to missing)
transform_into_missing <- names(df)[(unique_values < 300 & percentage_missing >= 10) & !(names(df) %in% cont_list)]
