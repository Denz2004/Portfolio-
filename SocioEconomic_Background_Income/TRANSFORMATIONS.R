...
numeric_cols <- names(new.rw.data)[sapply(new.rw.data, is.numeric)]
for (col in numeric_cols) {
  analyze_distribution(new.rw.data, col)
}

#based on the results we will log transform: W1GrssyrMP, W1GrssyrHH, W8QDEB2
variables_to_log_trans<-c("W1GrssyrMP(Gross annual salary of main parent)", "W1GrssyrHH(Gross annual salary of household)",
                          "W8QDEB2(Total amount owe)")
# Log-transform the chosen cols for the model that includes missing data, as these 3 cols aren't present in the main model
for (col in variables_to_log_trans) {
    model_with_missing[[col]] <- log(pmax(model_with_missing[[col]], 0.5))  # Log transform with pmax to avoid log(0) we have to justify this, in lectures they said to use 0.5
    ...
    colnames(model_with_missing)[colnames(model_with_missing) == col] <- new_col_name #change the name so we know which predictors got log transformed
  print(summary(model_with_missing[[new_col_name]])) #check changes
    }


#for both models, we will standardize all continuous columns so that they are easier to interpret (except for the target)
...
numeric_cols_no_target<- setdiff(numeric_cols, c("W8DINCW(Continuous weekly income)", 
                        "W8DGHQSC(General Health Questionnaire (GHQ12) score )", "W2ghq12scr(YP GHQ12 score)"))
numeric_cols_no_target
numeric_cols <- names(model_with_missing)[sapply(model_with_missing, is.numeric)]
numeric_cols

# Use the function from lectures
standardise.fn <- function(v) { (v - mean(v)) / sd(v) }

# Create a list containing both models
models_list <- list(model_with_missing, main_model)

# Loop through the models in the list
for (i in 1:length(models_list)) {
  model <- models_list[[i]]  # Access each model in the list
  
  for (col in numeric_cols_no_target) {
    # Standardize the non-target continuous columns
    model[[col]] <- standardise.fn(model[[col]])
    
    # Create the new column name
    new_col_name <- paste("standardised_", col, sep = "")
    
    # Add the standardized column with the new name
    model[[new_col_name]] <- model[[col]]
    
    # Remove the original column after standardizing
    model[[col]] <- NULL
    
    # Check changes by printing summary of the newly standardized column
    print(summary(model[[new_col_name]]))
  }
...
model_with_missing <- models_list[[1]]
main_model <- models_list[[2]]
