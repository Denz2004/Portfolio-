...
# Combine both into transform_into_missing
...
cor(new.rw.data$W1GrssyrMP, new.rw.data$W1GrssyrHH, use = "complete.obs")
#as they are moderately correlated, we will only include the gross household salary in the analysis, 
#similarly we drop empsdad but keep wrkfulldad
...
interesting_missing<-c("W1wrkfulldad(Whether father works full-time)", "W1GrssyrHH(Gross annual salary of household)",
...
#for W1wrkfulldad, the mean income for the missing data lands in between 2(working part-time) and 3(currently not working)

#a pattern is starting to emerge, the missing levels seem to have lower mean incomes compared to the highest levels
...
#we will now look to prove this by taking a different approach, analyzing the mean weekly income for when all 4 of the relevant columns all have missing data


# Identify rows where ALL actual_missing columns have "missing"
actual_missing<-c("W1wrkfulldad(Whether father works full-time)", 
...
sum(missing_all_rows) #checking our sample size of rows with missing values in all those columns, 47 is a small but still significant sample size

# Compute means for W8DINCW
mean_missing_all <- mean(new.rw.data$W8DINCW[missing_all_rows])
mean_not_missing_all <- mean(new.rw.data$W8DINCW[!missing_all_rows])
...
sd_from_mean <- (mean_missing_all - overall_mean) / overall_sd

#running a t test to show the missing data's impact on weekly income is significant and not random
t.test(new.rw.data$W8DINCW[missing_all_rows], 
       new.rw.data$W8DINCW[!missing_all_rows], 
       alternative = "less")

# Print results
print(paste("Number of rows missing across all 4 analysed columns:", sum(missing_all_rows)))
print(paste("Mean W8DINCW for rows missing in all 4 analysed columns:", round(mean_missing_all, 2)))
print(paste("Mean W8DINCW for rows not missing in all 4 analysed columns:", round(mean_not_missing_all, 2)))
...
ggplot(new.rw.data, aes(x = `W8DINCW(Continuous weekly income)`)) +
  geom_histogram(aes(fill = as.factor(missing_all_rows)), bins = 30, alpha = 0.6, position = "identity") +
  scale_fill_manual(values = c("blue", "red"), labels = c("Not Missing All", "Missing All")) +
  labs(title = "Income Distribution: Missing vs Not Missing All",
       x = "Weekly Income (W8DINCW)", 
       y = "Count", 
       fill = "Missing All Columns") +
  theme_minimal()
...
#the difference is significant too, with the mean being -1.74 SDs away from the actual mean of weekly income
#finally, the histogram reinforces our pattern; missing data implies a lower weekly income on average

#bias implications: Missing data is not random—it’s more common among lower-income individuals, 
